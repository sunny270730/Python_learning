...
class NumberConverter:
    def __init__(self):
        self.words_0_to_20 = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven', 'twelve', 'thirteen', 'fourteen', 'fifteen', 'sixteen', 'seventeen', 'eighteen', 'nineteen','twenty']
        self.words_21_to_99 = ['twenty', 'thirty', 'forty', 'fifty', 'sixty', 'seventy', 'eighty', 'ninety']

    def number_to_word(self, num):
        if num < 0 or num > 999:
            return " number out of range (0-9999)."
        if num < 20:
            return self.words_0_to_20[num]
        if num < 100:
            tens = self.words_21_to_99[(num // 10) - 2]
            ones = self.words_0_to_20[num % 10] 
            return tens + '-' + ones if num % 10 != 0 else tens
        if num < 1000:
            hundreds = self.words_0_to_20[num // 100] + ' hundred'
            tens = self.number_to_word(num % 100)
            return hundreds + ' and ' + tens if num % 100 != 0 else hundreds

converter = NumberConverter()
num = input("Enter the number 0 to 999:- ")
if num.isdigit():
    word = converter.number_to_word(int(num))
    print(word)
else:
    print('ERROR')
